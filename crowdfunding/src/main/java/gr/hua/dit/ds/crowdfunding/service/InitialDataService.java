...
import gr.hua.dit.ds.crowdfunding.repository.*;
...
public class InitialDataService {
    ...
    private void createProjects() {
        for (int i = 1; i <= LAST_PROJECT_ID; i++) {
            ...
            final String creator = faker.bool().bool() ? "creator" : "supporter";
            final Integer fundingGoal = faker.number().numberBetween(1000, 100000);
            final Integer currentFunding = faker.number().numberBetween(0, fundingGoal);

            this.projectRepository.findByName(name).orElseGet(() -> {
                Project project = new Project();
                project.setName(name);
                project.setDescription(description);
                project.setApproved(approved);
                project.setCreator(creator);
                project.setFundingGoal(fundingGoal);
